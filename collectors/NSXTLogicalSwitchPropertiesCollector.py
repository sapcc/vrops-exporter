...
class NSXTLogicalSwitchPropertiesCollector(PropertiesCollector):

    def __init__(self):
        super().__init__()
        self.vrops_entity_name = 'nsxt_logical_switch'
        self.label_names = ['nsxt_mgmt_cluster', 'nsxt_adapter', self.vrops_entity_name]
    ...
    def get_labels(self, resource_id, project_ids):
        return [self.nsxt_logical_switches[resource_id]['mgmt_cluster_name'],
                ...
                self.nsxt_logical_switches[resource_id]['name']] if resource_id in self.nsxt_logical_switches else []
