...
class TestLaunchExporter(TestCase):
    ...
    def test_with_cli_params_1(self):
        sys.argv = ['prog', '-o', '1234', '-d', '-i', 'inventory.some.url', '-m', 'collector_config.yaml']
        parse_params()
        ...
        self.assertEqual(os.getenv('CONFIG'), 'collector_config.yaml', 'Config was not set')

    # test with debug option off
    def test_with_cli_params_2(self):
        os.environ.clear()
        sys.argv = ['prog', '--port', '1234', '-i', 'inventory.some.url', '-m', 'collector_config.yaml']
        parse_params()
        self.assertEqual(os.getenv('PORT'), '1234', 'The port was not set correctly!')
    ...
    def test_with_cli_and_env_params(self):
        sys.argv = ['prog', '--port', '1234', "--debug", '-i', 'inventory.some.url', '-m', 'collector_config.yaml']
        os.environ['PORT'] = '1123'
        os.environ['INVENTORY'] = 'inventory.wedontwantthis.url'
        os.environ['CONFIG'] = 'collector_config.yaml'
        parse_params()
