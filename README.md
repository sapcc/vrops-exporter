...
The inventory retrieves all VMware resources by `name` and `uuid` from the vROps application interface, preserving the original parent-child relationships.         Likewise, it reads the atlas config file to know all the DNS names of the vrops targets. 
  ...
  * `--timeout`: specifies timeout for fetching data from vROps, default: 600s
  ...
  [atlas](https://github.com/sapcc/atlas) refers to our netbox extractor, which is in the end providing netbox data as a configmap in Kubernetes. You don't have to use it, a json file with the following structure would be sufficient, too.
   ...
   ```
In case the WSGI server can't be connected you might want to try `--loopback` to hook up the loopback interface (127.0.0.1).
